...
function getAlbums(req, res){
	...
	if (!artistId){
		...
		//Sacar los albums de un artista de la BBDD
...
function updateAlbum(req, res){
	...
	Album.findByIdAndUpdate(albumId, update, (err, albumUpdated) => {
		if (err){
			...
			if (!albumUpdated){
				res.status(404).send({message: 'El album no ha sido modificado'});
			}
			else {
...
function deleteAlbum(req, res){
	...
	Album.findByIdAndRemove(albumId,(err, albumRemoved) => {
		if (err){
			...
			if (!albumRemoved){
				res.status(404).send({message: 'El album no pudo eliminarse'});
			}
			else {
